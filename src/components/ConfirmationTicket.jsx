import "../scss/BookingConfirmationPage.scss"

const ConfirmationTicket = (props) => {

    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 336 101.93">
            <defs>
            </defs>
            <g id="Layer_2" data-name="Layer 2">
                <g id="Layer_1-2" data-name="Layer 1">
                    <path className="cls-1"
                        d="M327,0H98.76A15.23,15.23,0,0,1,86.47,6.22,15.23,15.23,0,0,1,74.18,0H9A9,9,0,0,1,0,9V92.89a9,9,0,0,1,9,9H74.17a15.26,15.26,0,0,1,24.6,0H327a9,9,0,0,1,9-9V9A9,9,0,0,1,327,0ZM86.47,93.71A2.53,2.53,0,1,1,89,91.18,2.53,2.53,0,0,1,86.47,93.71Zm0-8A2.53,2.53,0,1,1,89,83.13,2.53,2.53,0,0,1,86.47,85.66Zm0-8A2.53,2.53,0,1,1,89,75.13,2.53,2.53,0,0,1,86.47,77.66Zm0-8.11A2.53,2.53,0,1,1,89,67,2.53,2.53,0,0,1,86.47,69.55Zm0-8A2.53,2.53,0,1,1,89,59,2.53,2.53,0,0,1,86.47,61.55Zm0-8A2.53,2.53,0,1,1,89,51,2.53,2.53,0,0,1,86.47,53.55Zm0-8A2.53,2.53,0,1,1,89,43,2.53,2.53,0,0,1,86.47,45.55Zm0-8.22A2.53,2.53,0,1,1,89,34.8,2.53,2.53,0,0,1,86.47,37.33Zm0-8A2.53,2.53,0,1,1,89,26.8,2.53,2.53,0,0,1,86.47,29.33Zm0-8A2.53,2.53,0,1,1,89,18.8,2.53,2.53,0,0,1,86.47,21.33Zm0-8A2.53,2.53,0,1,1,89,10.8,2.53,2.53,0,0,1,86.47,13.33Z" />
                    <text className="cls-2" transform="translate(104.62 22.76)">D<tspan className="cls-3" x="13.4" y="0">o</tspan>
                        <tspan className="cls-4" x="24.38" y="0">r</tspan>
                        <tspan className="cls-3" x="30.82" y="0">a the Explo</tspan>
                        <tspan className="cls-5" x="122.86" y="0">r</tspan>
                        <tspan className="cls-6" x="129.2" y="0">er</tspan>
                    </text><text className="cls-7" transform="translate(105.12 37.33)">
                        <tspan className="cls-8">A</tspan>
                        <tspan className="cls-9" x="7.2" y="0">c</tspan>
                        <tspan x="12.73" y="0">tion / PG 13</tspan>
                    </text><text className="cls-10" transform="translate(104.47 75.13)">
                        <tspan className="cls-11">B</tspan>
                        <tspan x="7.64" y="0">e</tspan>
                        <tspan className="cls-12" x="14.66" y="0">t</tspan>
                        <tspan className="cls-13" x="19.21" y="0">ongsalongen</tspan>
                    </text><text className="cls-10" transform="translate(105.53 91.18)">2021/05/12 19:00-21:00</text><text
                        className="cls-14" transform="translate(292.12 45.55)">
                        <tspan className="cls-15">S</tspan>
                        <tspan className="cls-16" x="8.98" y="0">e</tspan>
                        <tspan className="cls-17" x="18" y="0">a</tspan>
                        <tspan x="26.6" y="0">t</tspan>
                        <tspan className="cls-18">
                            <tspan x="0.39" y="21.6">A36</tspan>
                        </tspan>
                    </text>
                </g>
            </g>
        </svg>
    );
}

export default ConfirmationTicket;